...
function [marg_y,out] = get_marginal_predictive(Y1, Y2, Y3, Y4, K1, K2, K3, K4, s1, s2, s3, s4,eps,tau1, tau2, tau3, tau4, l1, l2, l3, l4,EB)
    % Assumes Y has been scaled on the interval (0,1].
    %   but doesn't throw an error at the moment.
    %warning('off','MATLAB:nearlySingularMatrix')
    %max_cntr_EM = 100;
    %tol_EM = 1e-9;
    [ns1, ab1, Z1] = get_vals(Y1, s1, K1);
    [ns2, ab2, Z2] = get_vals(Y2, s2, K2);
    [ns3, ab3, Z3] = get_vals(Y3, s3, K3);
    [ns4, ab4, Z4] = get_vals(Y4, s4, K4);
    ...
    %else
    [f,marg_y,Omegainv] = get_f_predictive(ns1, ns2, ns3, ns4, ab1, ab2, ab3, ab4, Z1, Z2, Z3, Z4, tau(1), tau(2), tau(3), tau(4), l(1), l(2), l(3), l(4), eps);
    %end
...
% end
% 
% function binind = get_binind(n, Y, s, K)
%     binind = zeros(n,1);
%     for ii=1:n
%         %Y(ii,1) < s(2:(K+1))
...
% end
% 
% function [s, K] = reassign_s_k(s, maxbin)
%     s = s(1:(maxbin+1));
%     K = maxbin;
% end
% 
% function Z = get_Z(s, K) 
%     Z = (s(1:K) + diff(s)/2)';
% end



function [ns, ab, Z] = get_vals(Y, s, K)
    n = size(Y,1);
    if(isempty(s))
        %s = linspace(0,max(Y(:,1)) + min(.1,max(Y(:,1))/100),K+1);
        s = linspace(0,1.01,K+1);
